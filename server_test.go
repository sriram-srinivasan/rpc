...
const (
	newHttpPath    = "/foo"
	asyncDelayTime = 50 * time.Millisecond
)
...
func (t *Arith) Add(args Args, reply *Reply) error {
	if args.ReplyAsync {
		f := NewFuture()
		go func() {
			time.Sleep(asyncDelayTime)
			f.Set(&Reply{args.A + args.B}, nil)
		}()
		return f
	} else {
...
func (t *Arith) Mul(args *Args, reply *Reply) error {
	if args.ReplyAsync {
		// unlike Arith.Add, create a Future, but set it before returning.
		f := NewFuture()
		f.Set(&Reply{args.A * args.B}, nil)
		return f
	} else {
...
func (t *Arith) Div(args Args, reply *Reply) error {
	if args.ReplyAsync {
		...
		go func() {
			time.Sleep(asyncDelayTime)
			if args.B == 0 {
				f.Set(nil, errors.New("divide by zero"))
			} else {
				f.Set(&Reply{args.A / args.B}, nil)
			}
		}()
...
func testRPC(t *testing.T, addr string) {
	...
	start := time.Now() // to ensure that the call did indeed take the delayed route
	err = client.Call("Arith.Add", args, reply)
	if time.Since(start) < asyncDelayTime {
		t.Errorf("Async delay expected. Reply received in %v", time.Since(start))
	}
	if err != nil {
		t.Errorf("Add: expected no error but got string %q", err.Error())
	}
	...
	err = client.Call("Arith.Div", args, reply)
	if time.Since(start) < asyncDelayTime {
		t.Error("Div: expected async call to delay by a second")
	}
	// expect an error: zero divide
	if err == nil {
		t.Error("Div: expected error")
	...
	// Async test where the Future's values are set before the method returns
	// See Arith.Mul
	args = &Args{7, 8, true}
	reply = new(Reply)
	err = client.Call("Arith.Mul", args, reply)
...
func TestErrorAfterClientClose(t *testing.T) {
	...
	err = client.Call("Arith.Add", &Args{7, 9, false}, new(Reply))
	if err != ErrShutdown {
